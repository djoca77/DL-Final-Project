...
class Conv2D(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        #input_shape to access how many channels
        #kernel_size x inputshape last channel length x self.filters
        
        self.kernel = self.kernel_initializer([self.kernel_size[0],self.kernel_size[1],input_shape[-1],self.filters])
        self.kernel = tf.Variable(initial_value=self.kernel)
        self.bias = self.bias_initializer([1,self.filters])
        self.bias = tf.Variable(initial_value=self.bias)
    ...
    def call(self, inputs, training=False):
        ...
        out = tf.nn.convolution(inputs,filters=self.kernel,strides=self.strides,padding=self.padding)
        if self.use_bias:   
            out = out + self.bias
        
        layer_out = self.activation(out)
        ...
        self.add_loss(self.activity_regularizer(layer_out))
...
class BatchNormalization(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.beta = self.beta_initializer(self.scale)
        self.gamma = self.gamma_initializer(self.center)



        self.moving_mean = self.moving_mean_initializer(input_shape[self.axis])
        self.moving_variance = self.moving_variance_initializer(input_shape[self.axis])

        ## TODO 2: Initialize components as tf.Variables; specify trainable status as appropriate
        self.beta = tf.Variable(initial_value=self.beta,trainable=True)
        self.gamma = tf.Variable(initial_value=self.gamma,trainable=True)
        self.moving_mean = tf.Variable(initial_value=self.moving_mean,trainable=False)
        self.moving_variance = tf.Variable(initial_value=self.moving_variance,trainable=False)
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            ...
            norm = (inputs - bmu)/tf.sqrt(bvar+self.epsilon)
            print(bmu.shape)
            ...
            norm = (inputs - self.moving_mean)/tf.sqrt(self.moving_variance+self.epsilon)
...
class Dropout(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        if training:
            dropper = tf.random.uniform(tf.shape(inputs),seed=self.seed)
            dropper = dropper > self.rate
            dropper = tf.cast(dropper,dtype=tf.float32)

            dropped = (dropper*inputs)/(1 - self.rate)
